...
const SignsVisualSettings = ({userPreferencesActions,userPreferences}) => {
    ...
    const handleSignsSizeAfterChange = (value) => {
        userPreferencesActions.changeVisualPreference('signs','signsSize',value);
    }
    ...
    const handleZoomAfterChange = (valueMin, valueMax) => {
        userPreferencesActions.changeVisualPreference('roads','signsZoomFrom',valueMin);
        userPreferencesActions.changeVisualPreference('roads','signsZoomTo',valueMax);
    }

    const initFromProps = () => {
        setSignsSize(userPreferences && userPreferences.signsSize ? userPreferences.signsSize : 100);
        setZoomMinSignsRender(userPreferences && userPreferences.zoomMinSignsRender ? userPreferences.zoomMinSignsRender : 5);
        setZoomMaxSignsRender(userPreferences && userPreferences.zoomMaxSignsRender ? userPreferences.zoomMaxSignsRender : 14);
    }
